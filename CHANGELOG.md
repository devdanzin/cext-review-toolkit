...
### Fixed
- **`tree_sitter_utils.extract_functions`: descend into `#ifndef`/`#ifdef`/`#if` preprocessor guards** — the top-level node collection walked only `root.children` (plus `extern "C"`/`namespace` bodies), so a **header-only C extension** whose entire body sits inside an `#ifndef FOO_H … #endif` include guard parses as a single `preproc_ifdef` node and yielded **zero functions**. Every AST-based scanner (refcount, error-path, null-safety, type-slot, resource-lifecycle, pyerr-clear, complexity) was therefore blind on such extensions. Now flattens recursively through `preproc_ifdef`/`preproc_if`/`preproc_elif`/`preproc_else` (and nested combinations, e.g. an `#if PY_VERSION_HEX` inside the guard). Surfaced by an aio-libs/multidict review where 100% of the logic lives in `_multilib/*.h`: `extract_functions` went 62→203 functions, `scan_refcounts` 6→40 candidates (all header findings had to be recovered by hand before). Three regression tests added (`test_tree_sitter_utils.py`: include-guard body, nested `#if` conditional, no-guard-unaffected). Known follow-up: the descent now also surfaces candidates inside a vendored `pythoncapi_compat.h`; a filename-based vendored-shim skip would cut that noise.

### Added
- **`scan_error_paths.py`: `tristate_bool_confusion` detection** — flags tri-state int APIs (return `-1` on error, `0`/`1` otherwise: `PyObject_IsTrue`, `PyObject_Not`, `PyObject_RichCompareBool`, `PyObject_IsInstance`/`IsSubclass`, `PySequence_Contains`, `PyDict_Contains`, `PySet_Contains`, the `*WithError` has-key/has-attr variants) whose result is used as a boolean without a `< 0` check. C's logical operators treat `-1` as true, so `x && PyObject_IsTrue(y)`, `if (PyObject_IsTrue(y))`, `!PyObject_IsTrue(y)`, or storing the result and testing it without `< 0` silently misreads the error case → a result returned with a pending exception (`SystemError`) or corrupted state. Flags `&&`/`||`/`!` uses as **high** confidence and condition-truthiness / stored-without-check as **medium**; treats compared (`< 0`, `== -1`, …), returned (propagated), and `< 0`-checked results as safe. Closes a gap surfaced by coveragepy `tracer.c:987` (coveragepy PR #2219 issue #2), and previously hit manually in cereggii (#15) and zstandard (F36). Four new tests (logical-`&&`, stored-unchecked, if-condition, `< 0`-checked true-negative).

### Documentation
- **Reproducer catalog T30 — `python -O` probe for assert-based validation** in `docs/reproducer-techniques.md`. Diff-runs the same code under default and `-O` interpreters to surface assertions used as input validation that silently disappear in production mode. Covers the Python side (`assert` stripped under `-O`) and the C side (`assert()` stripped when `NDEBUG` is defined in release wheels). Includes a runnable subprocess-based diff script, five gotchas (internal invariant vs external input, `-OO` vs `-O`, import-time asserts, `sys.flags.optimize`, downstream non-determinism), an explicit fix recipe (`assert` → `raise`), and a confirmed-on entry for VTK 9.6.1 `vtkmodules/util/numpy_support.py` (4 input-validation asserts at lines 134, 135, 137, 218 — complex arrays silently float-cast, 3D arrays silently accepted, non-contiguous slices silently accepted; `vtkTypeFloat64Array` instances flow into VTK C++ with truncated/wrongly-laid-out memory). The README documentation index is bumped to "30 numbered patterns" to match.
- **Reproducer catalog T31 — libfiu-free malloc-fault injection via `LD_PRELOAD`** in `docs/reproducer-techniques.md`, with a reusable kit: `docs/mallocfault.c` (armable malloc/calloc/realloc fault shim — starts disarmed, armed via a sentinel file around exactly the targeted call; after-N/every-K/count/size-window controls), `docs/libxmlcount.c` (worked example of an event-arming create/free counter that proves leaks directly and flips the fault at a precise C call boundary; adapt the wrapped symbols per target library), and `docs/mallocfault_harness.py` (generic `run_isolated`/`arm`/`disarm`/`leak_probe`/`deadlock_probe` driver). Complements Technique 23 (libfiu) for hosts without libfiu and for pinpointing one buried allocation. Confirmed on lxml 7.0.0a2 (6 OOM lock-leak/deadlock findings reproduced; one finding corrected as unreachable — a `noexcept` callee).
- **Reproducer catalog T32 — interpreting TSan signals on free-threaded (3.14t) builds** in `docs/reproducer-techniques.md`. Documents that CPython 3.14t's thread-safe dict/list internals change how shared-state races manifest: a `dict` race shows as *logical* corruption (`RuntimeError`/`KeyError`/wrong results) with **no** TSan line, while `list` mutate-during-iterate, use-after-free, and adjacent-refcount races *do* surface (the last can crash the interpreter). Includes how to drive genuinely-shared state for a clean signal. Confirmed on lxml 7.0.0a2 (all four manifestations). The README documentation index is bumped to "32 numbered patterns".
