...
def _detect_code_generation(root: Path, c_files: list[str]) -> str:
    """Detect if C source files are generated by a known tool.

    Returns one of: "hand_written", "cython", "mypyc", "pybind11", "mixed".
    """
    has_cython = False
    has_mypyc = False
    has_pybind11 = False
    ...
    # Check for .pyx files as a Cython indicator.
    pyx_files = list(root.rglob("*.pyx"))
    pyx_files = [p for p in pyx_files if not _should_skip(p, root)]
    if pyx_files:
        has_cython = True
    ...
    for rel_path in c_files:
        ...
        if "Generated by Cython" in header_text:
            has_cython = True
        elif "CPyDef_" in header_text or "mypyc" in header_text.lower():
            has_mypyc = True
        elif "PYBIND11_MODULE" in header_text or "py::class_" in header_text:
            has_pybind11 = True
        else:
            # Also check for mypyc/pybind11 patterns beyond the header.
            try:
                content = full_path.read_text(encoding="utf-8", errors="replace")
            except OSError:
    ...
    if not generators:
        return "hand_written"
    if has_hand_written and generators:
        return "mixed"
    if len(generators) > 1:
        return "mixed"
    return generators[0]
